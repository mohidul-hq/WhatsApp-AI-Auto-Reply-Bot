...
function buildPrompt(senderName, incomingMessage, history = [], userMeta = {}) {
    ...
    const interests = userMeta.interests?.join(', ') || 'none';

    return `
You're Mohidul, a teenage Indian boy acting as a friendly WhatsApp assistant. Speak like a real desi teenager in casual Hinglish or Banglish (mix Hindi, Bengali, and English in roman script). Keep replies short, funny or romantic (if needed), and always in WhatsApp-style — just like you're texting a friend. Avoid sounding robotic or formal.
...
5. Act like a smart, friendly teenage boy. Use Hinglish/Banglish, emojis, and slang. Be funny, and relatable, romantic if needed.
...
- Simulate small typos or corrections occasionally.
`;}

async function generateAIReply(senderName, senderNumber, incomingMessage) {
    try {
        const history = chatHistory.get(senderNumber) || [];
        const userMeta = await ChatModel.findOne({ number: senderNumber }) || {};
        const prompt = buildPrompt(senderName, incomingMessage, history, userMeta);
...
function getSeenDelay(senderNumber) {
    ...
    return timeSinceLast < 30000 ? Math.floor(Math.random() * 10000) + 3000 : Math.floor(Math.random() * 30000) + 10000;
}
    ...
    setTimeout(async () => {
        ...
        setTimeout(async () => {
            ...
            await ChatModel.updateOne(
                { number: senderNumber },
                {
                    $push: { messages: { from: 'user', text: message.body, timestamp: new Date() } }
                },
                { upsert: true }
            );
