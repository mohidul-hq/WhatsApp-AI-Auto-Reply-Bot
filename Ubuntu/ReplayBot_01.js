...
// Admin numbers (add in international format without '+' sign)
...
// Utility: Format number if no name found
...
// Build conversational prompt with sender's name
...
async function processQueue() {
    ...
    // Get sender name from WhatsApp contact info
    const senderName = contact.pushname || contact.name || formatNumber(contact.number);
    const senderNumber = contact.number.replace(/\D/g, '');

    const seenDelay = Math.floor(Math.random() * 40000) + 2000;
